...
이 스킬이 설계된 특정 프로젝트에서 작업할 때 참조하세요. 프로젝트 스킬에 포함되는 것:
- 아키텍처 개요
- 파일 구조
- 코드 패턴
- 테스트 요구사항
...
- **AI**: 도구 호출 및 구조화된 출력을 가진 Claude API
- **배포**: Google Cloud Run
...
1. **이모지 없음** 코드, 주석, 문서에
2. **불변성** - 객체나 배열 절대 변이 안함
3. **TDD** - 구현 전에 테스트 작성
4. **80% 커버리지** 최소
5. **많은 작은 파일** - 200-400줄 일반적, 800 최대
6. **console.log 없음** 프로덕션 코드에
7. **적절한 오류 처리** try/catch로
8. **입력 검증** Pydantic/Zod로
